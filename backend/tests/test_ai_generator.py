...
class TestAIGenerator:
    ...
    def setup_class(cls):
        ...
        cls.has_api_key = bool(
            config.ANTHROPIC_API_KEY
            and config.ANTHROPIC_API_KEY != "your_anthropic_api_key_here"
        )

        if cls.has_api_key:
            cls.ai_generator = AIGenerator(
                config.ANTHROPIC_API_KEY, config.ANTHROPIC_MODEL, config.MAX_TOOL_ROUNDS
            )
    ...
    def test_sequential_tool_calling(self):
        ...
        try:
            ...
            mock_final_response = MockAnthropicResponse(
                "Comprehensive answer using both tools"
            )
    ...
    def test_sequential_tool_calling_mock(self):
        ...
        try:
            ...
            # Mock final response
            mock_final_response = MockAnthropicResponse(
                "Complete answer combining outline and detailed content"
            )
    ...
    def test_tool_round_limit_enforcement(self):
        ...
        try:
            ...
            mock_final_response = MockAnthropicResponse(
                "Final response after tool limit reached"
            )
