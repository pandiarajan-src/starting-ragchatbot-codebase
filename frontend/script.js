...
function addMessage(content, type, sources = null, isWelcome = false) {
    ...
    if (sources && sources.length > 0) {
        // Format sources as clickable links where available
        const formattedSources = sources.map(source => {
            // Check if source is object with text property
            if (source && typeof source === 'object' && source.text) {
                // New format with link information
                if (source.link) {
                ...
                // Legacy format - just text, or fallback
                return `<span class="source-text">${String(source)}</span>`;
            }
        }).join(' ');
...
function toggleTheme() {
    ...
    // Add a small animation feedback to the button
    if (themeToggle) {
